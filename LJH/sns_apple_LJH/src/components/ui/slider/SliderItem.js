      ...
      <article className="post post--horizontal post--horizontal-reverse takei-slide-featured">
        <div className="post__thumb">
          <div
            ...
            style={{ background: `url('${postItem.titleImage}')` }}
          ></div>
          <Link to={`/single/${postId}`} className="link-overlay"></Link>
        </div>
        <div className=" post-text__wrap">
          <div className="post__text">
            <Link to="category-1.html" className="post__cat takei-cat">
              {cateName}
            </Link>
            <h3 className="post__title takei-title typescale-5">
              {" "}
              <Link to={`/single/${postId}`}>{postItem.postTitle}</Link>
            </h3>
          </div>
        </div>
      </article>
